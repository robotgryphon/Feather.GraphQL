...
public sealed class QueryInterceptorGenerator : IIncrementalGenerator
{
    ...
    /// <remarks>
    /// <c>Owner</c> and <c>Caller</c> are where the chain was written — the type and the method.
    /// Neither changes what is generated; they decide what it is called and which file it lands
    /// in, so that generated code sits beside the code that caused it rather than in one pile
    ...
    private static Interception? Describe(GeneratorSyntaxContext context, CancellationToken token)
    {
        ...
        return Signature(method, location.GetInterceptsLocationAttributeSyntax(), document,
            Plan(completions, context.SemanticModel, token), Owner(invocation), Caller(invocation));
    }
    ...
    /// <summary>The type the chain was written in, as a name a file can be called.</summary>
    ...
    /// <summary>The method the chain was written in, as a name another method can be called.</summary>
    private static string Caller(SyntaxNode node)
    {
        for (var current = node.Parent; current is not null; current = current.Parent)
        {
            switch (current)
        ...
        string caller)
    {
        ...
        return new Interception(attribute, typeParameter, Constraints(definition.TypeParameters[0]),
            string.Join(", ", parameters), @using, forward, document, plan, owner, caller);
    }
    ...
    private static void Emit(SourceProductionContext context, ImmutableArray<Interception> found)
    {
        ...
        // A file per type the chains were written in, rather than one for the whole compilation.
        // The class is named for that type and each method for the one it was written in, so a
        // generated interceptor is findable from the code that caused it — and an error in one
        // names something the author recognises instead of Query17.
        int index = 0;

        foreach (var group in found.Distinct().GroupBy(x => x.Owner, StringComparer.Ordinal))
        {
            ...
            foreach (string @using in owned.Select(x => x.Using)
                .Where(x => x.Length > 0).Distinct().OrderBy(x => x, StringComparer.Ordinal))
            {
                builder.Append("    using global::").Append(@using).Append(";\n");
            }

            builder.Append("    /// <summary>\n")
                .Append("    /// The queries written in <c>").Append(group.Key)
                .Append("</c> whose documents the compiler could print. Each replaces the call that\n")
                .Append("    /// starts its chain, and does nothing but hand the chain that document.\n")
                .Append("    /// </summary>\n")
                .Append("    file static class ").Append(group.Key).Append("_Precompiled\n")
                .Append("    {\n");

            // The filters are siblings rather than nested, because a file-local type cannot nest.
            var filters = new StringBuilder();

            foreach (var interception in owned)
            {
                string name = interception.Caller + index;

                builder.Append("        ").Append(interception.Attribute).Append('\n')
                    .Append("        public static global::System.Linq.IQueryable<")
                    .Append(interception.TypeParameter).Append("> ").Append(name)
            ...
            context.AddSource(
                "GraphQLPrecompiled." + group.Key + ".g.cs",
