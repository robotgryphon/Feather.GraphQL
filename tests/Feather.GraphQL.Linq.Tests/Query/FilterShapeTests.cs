...
public class FilterShapeTests
{
    ...
    {
        var plan = GraphQLQueryable.For<Country>()
    ...
    public void The_filter_input_type_name_comes_from_the_queried_type()
        => Assert.That(GraphQLQueryable.For<Country>()
                .Where((CountryFilter f) => f.Continent == "Europe")
                .Select(c => new { c.Name })
                .ToQueryPlan().Query,
    ...
    public void Field_names_honour_JsonPropertyName_on_the_filter_shape()
        => Assert.That(Variables(GraphQLQueryable.For<Country>()
    ...
    public void Mixing_the_two_shapes_is_FGQL020()
        => Assert.That(ThrowsWith(() => GraphQLQueryable.For<Country>()
    ...
    {
        string inlineLast = GraphQLQueryable.For<Person>()
            .WithGraphQLArguments(new() { Filter = "criteria" })
            .Where("filter", p => p.Age > 30)
            .ToQueryPlan().Query;

        string argumentsLast = GraphQLQueryable.For<Person>()
            .Where("filter", p => p.Age > 30)
            .WithGraphQLArguments(new() { Filter = "criteria" })
            .ToQueryPlan().Query;
    ...
    public void An_empty_inline_name_is_rejected()
        => Assert.Throws<ArgumentException>(
            () => GraphQLQueryable.For<Person>().Where(" ", p => p.Age > 30));
