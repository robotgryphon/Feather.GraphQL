...
public class ProjectionAnalyzerTests
{
    ...
    public void A_member_leading_back_into_the_projection_is_fine()
        => Assert.That(
            AnalyzerHarness.Diagnose("countries.Select(c => new { c.Name, c.Continent });"),
            Is.Empty);
    ...
    public void A_member_with_no_scalars_is_FGQL014()
        => Assert.That(
            AnalyzerHarness.Diagnose("countries.Select(c => new { c.Parts });"),
            Is.EqualTo(new[] { "FGQL014" }));

    [Test]
    public void An_object_member_of_scalars_is_fine()
        => Assert.That(
            AnalyzerHarness.Diagnose("countries.Select(c => new { c.Name, c.Size });"),
            Is.Empty);

    [Test]
    public void A_scalar_list_is_fine()
        => Assert.That(
            AnalyzerHarness.Diagnose("countries.Select(c => new { c.Tags });"),
            Is.Empty);
    ...
    public void Naming_the_nested_fields_clears_it()
        => Assert.That(
            AnalyzerHarness.Diagnose("countries.Select(c => new { c.Name, Continent = c.Continent.Name });"),
            Is.Empty);
    ...
    public void A_nested_Select_clears_it()
        => Assert.That(
            AnalyzerHarness.Diagnose(
                "countries.Select(c => new { Parts = c.Parts!.Primary.Select(p => new { p.Name }).ToArray() });"),
            Is.Empty);
    ...
    public void A_chain_without_a_projection_expands_its_elements()
        => Assert.That(
            AnalyzerHarness.Diagnose("countries.Select(c => new { Parts = c.Parts!.Primary.ToArray() });"),
            Is.Empty);

    [Test]
    public void An_object_initializer_is_walked_too()
        => Assert.That(
            AnalyzerHarness.Diagnose("countries.Select(c => new Continent { Name = c.Continent.Name });"),
            Is.Empty);

    [Test]
    public void A_constructor_argument_is_walked_too()
        => Assert.That(
            AnalyzerHarness.Diagnose("countries.Select(c => new System.Tuple<Parts>(c.Parts!));"),
            Is.EqualTo(new[] { "FGQL014" }));
    ...
    public void Each_offending_member_is_reported_once()
        => Assert.That(
            AnalyzerHarness.Diagnose("countries.Select(c => new { c.Continent, c.Parts, c.More });"),
